...
| Emscripten | static `.bc` linked into frontend | `emscripten` | Pinned RetroArch JS/WASM build, HTTP checks, synthetic rendering, authentic auto-load, menu navigation, Automatic Story visual/water-motion plus connected-component color-key gates pass. Clean candidate `02e8ddb` also passes three consecutive strict gap-free delayed-unlock gameplay windows, exact intentional menu silence, unlock/reset persistence, and a separate ordinary full-options run. The v2 all-63 matrix applies the ordinary strict window to 61 chapters and automatically applies reset-scoped, chapter-local actor-motion windows to short `stand15`/`stand16`; physical browser/device execution remains candidate-specific |
